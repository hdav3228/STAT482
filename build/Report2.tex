...
This correlation is worth monitoring with Variance Inflation Factors (VIFs) in the modeling stage so that multicollinearity does not distort the coefficient estimates.
Release speed and horizontal break are also negatively correlated ($r = -0.30$), reflecting the tendency for slower breaking pitches to move more laterally.
...
This confirms that no single physical characteristic is a dominant predictor of swing-and-miss outcomes, which reinforces the need for richer models that incorporate sequencing and batter heterogeneity.
