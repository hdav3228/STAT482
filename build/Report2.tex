...
\newcommand{\glmmSigma}{0.549}
\newcommand{\glmmSigmaSq}{0.301}
\newcommand{\glmmICC}{8.4}
...
Taken together, these results support the core sequencing hypothesis: whiff probability depends not just on what pitch is thrown, but on what the batter just saw.
...
For computational tractability, the GLMM is fit on a random subsample of \glmmObs{} pitches from the sequencing dataset, spanning \glmmBatters{} batters and \glmmFixed{} fixed-effect terms.
...
That result matters because it shows that batter identity is not just a nuisance grouping factor: even after controlling for pitch physics and pitch context, hitters still differ systematically in baseline swing-and-miss tendency.
...
Figure~\ref{fig:model_compare} shows the fit comparison across the three models estimated on the common full-sample likelihood framework, and Figure~\ref{fig:combined_re} plots the estimated batter random intercepts from the combined model.
...
The standalone GLMM then shows that batter-level heterogeneity remains material even after introducing those pitch-context covariates: the estimated ICC of \glmmICC\% implies that batter identity explains a meaningful share of latent whiff variation.
Once batter-specific random intercepts are added directly to the sequencing specification, the combined model captures that additional source of systematic variation and becomes the best-fitting specification by a substantial margin.
...
That combined approach gives the best fit among the directly comparable models and yields the most realistic picture of how swing-and-miss outcomes arise over the course of a plate appearance.
